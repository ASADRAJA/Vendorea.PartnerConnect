...
public class EdiDocumentsController : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation(
            ...
            connectionId, file.FileName);

        using var reader = new StreamReader(file.OpenReadStream());
        var content = await reader.ReadToEndAsync(cancellationToken);

        var result = await _processingService.ProcessDocumentAsync(
            connectionId, content, file.FileName, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await _processingService.ProcessDocumentAsync(
            connectionId, request.EdiContent, fileName, cancellationToken);
    ...
    public async Task<IActionResult> GetByConnection(
        int connectionId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var documents = await _processingService.GetDocumentsAsync(
            connectionId, transactionSetCode, directionEnum, skip, take, cancellationToken);
    ...
    public async Task<IActionResult> GetPendingOutbound(
        ...
        CancellationToken cancellationToken = default)
    {
        var documents = await _responseService.GetPendingResponsesAsync(connectionId, cancellationToken);
        return Ok(documents.Select(MapToDto));
    }
    ...
    /// </summary>
    [HttpPost("connection/{connectionId:int}/send-pending")]
    [ProducesResponseType(typeof(EdiBatchSendResult), StatusCodes.Status200OK)]
    public async Task<IActionResult> SendPending(
        int connectionId,
        CancellationToken cancellationToken = default)
    {
        var result = await _responseService.SendPendingResponsesAsync(connectionId, cancellationToken);
        return Ok(result);
    }
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Sync(int connectionId, CancellationToken cancellationToken = default)
    {
        var connection = await _connectionRepository.GetByIdAsync(connectionId, cancellationToken);
        if (connection == null)
        {
        ...
        _logger.LogInformation("Manually triggered EDI sync for connection {ConnectionId}", connectionId);

        var result = await _processingService.SyncEdiDocumentsAsync(connectionId, cancellationToken);
        return Ok(result);
    }
    ...
    /// </summary>
    [HttpGet("connection/{connectionId:int}/stats")]
    [ProducesResponseType(typeof(Dictionary<string, int>), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetStats(
        int connectionId,
        CancellationToken cancellationToken = default)
    {
        var counts = await _ediDocumentRepository.GetCountsByTransactionSetAsync(connectionId, cancellationToken);
        return Ok(counts);
    }
